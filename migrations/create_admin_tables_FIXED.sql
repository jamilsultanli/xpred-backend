-- =====================================================
-- COMPREHENSIVE ADMIN PANEL DATABASE SCHEMA - FIXED
-- =====================================================
...
-- =====================================================
-- DROP EXISTING ADMIN TABLES (IF NEEDED FOR CLEAN INSTALL)
-- =====================================================
...
-- =====================================================
-- AUDIT LOGS TABLE (FIXED)
-- =====================================================
-- Drop and recreate if structure is different
DO $$
BEGIN
  -- Check if audit_logs exists and if it has the correct structure
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'audit_logs') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
      ...
      RAISE NOTICE 'Dropped old audit_logs table with incompatible structure';
    END IF;
  END IF;
END $$;
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_audit_logs_admin') THEN
    CREATE INDEX idx_audit_logs_admin ON audit_logs(admin_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_audit_logs_created') THEN
...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_system_settings_public') THEN
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_admin_user_notes_user') THEN
    CREATE INDEX idx_admin_user_notes_user ON admin_user_notes(user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_admin_user_notes_admin') THEN
    CREATE INDEX idx_admin_user_notes_admin ON admin_user_notes(admin_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_admin_user_notes_created') THEN
...
-- =====================================================
CREATE TABLE IF NOT EXISTS prediction_resolution_queue (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_resolution_queue_prediction') THEN
    CREATE INDEX idx_resolution_queue_prediction ON prediction_resolution_queue(prediction_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_resolution_queue_status') THEN
    CREATE INDEX idx_resolution_queue_status ON prediction_resolution_queue(status);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_resolution_queue_submitted_by') THEN
    CREATE INDEX idx_resolution_queue_submitted_by ON prediction_resolution_queue(submitted_by);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_resolution_queue_created') THEN
...
-- =====================================================
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS admin_role_id UUID REFERENCES admin_roles(id);
...
DROP POLICY IF EXISTS admin_user_notes_read ON admin_user_notes;
DROP POLICY IF EXISTS resolution_queue_read ON prediction_resolution_queue;
DROP POLICY IF EXISTS resolution_queue_insert ON prediction_resolution_queue;
...
BEGIN
  ...
  RAISE NOTICE '📊 Tables: admin_roles, admin_permissions, audit_logs, system_settings, admin_user_notes, prediction_resolution_queue';
  ...
  RAISE NOTICE '';
  RAISE NOTICE '🚀 Next step: Promote your first admin user';
  RAISE NOTICE 'Example SQL:';
  RAISE NOTICE '  UPDATE profiles SET role = ''admin'', admin_role_id = (SELECT id FROM admin_roles WHERE name = ''super_admin'') WHERE email = ''your@email.com'';';
END $$;
